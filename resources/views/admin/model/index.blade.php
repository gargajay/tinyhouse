                ...
                <div class="page-header-content header-elements-lg-inline">
                    ...
                    <div class="header-elements d-none">
                        <div class="d-flex justify-content-center">
                            ...
                            <a href="{{ route('model.create') }}" type="button" class="btn btn-primary">Add</a>

                            <form style="margin-left: 5px;" action="{{ route('model.index') }}">
                                ...
                                @if (count($result) > 0)
                                    ...
                                    @foreach ($result as $key => $value)
                                        <tr>
                                            ...
                                            <td>{{ $value['years']['year'] ?? '' }}</td>
                                            <td>{{ $value['Carmake']['name'] ?? '' }}</td>
                                            <td>{{ $value['name'] ?? '' }}</td>
                                           
                                            <td>
                                                <div class="list-icons">
                                                    <a href="{{ route('model.edit', [$value['id']]) }}"
                                                        class="list-icons-item text-primary" title="Edit"><i
                                                            class="icon-pencil7"></i></a>
                                                    <a href="{{ route('model.show', [$value['id']]) }}"
                                                        class="list-icons-item text-danger delete-confirm" title="Delete"><i
